...
namespace nandish_ICA08
{
  internal class Ball
  {
    ...
    //show function to render balls with number
    public void Show(CDrawer drawer, int num)
    {
      drawer.AddCenteredEllipse((int)bCenter.X, (int)bCenter.Y, bRadius * 2, bRadius * 2, _color);
      drawer.AddText(num.ToString(), 15, (int)bCenter.X - bRadius, (int)bCenter.Y - bRadius, bRadius * 2, bRadius * 2);
    }
  }
}
