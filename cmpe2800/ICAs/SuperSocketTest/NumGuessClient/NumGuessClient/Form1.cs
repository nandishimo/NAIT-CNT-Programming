...
namespace NumGuessClient
{
	public partial class Form1 : Form
	{
		...
		private void SetTrackbarLimits(int min = 1, int max = 1000)
		{
			try
			{
				Invoke(new Action(() => { UI_Tbar_Guess.Minimum = min; }));
				Invoke(new Action(() => { UI_Tbar_Guess.Maximum = max; }));
				Invoke(new Action(() => { UI_Tbox_Min.Text = min.ToString(); }));
				Invoke(new Action(() => { UI_Tbox_Max.Text = max.ToString(); }));
			}
			catch (Exception ex)
			{
				WriteLine(ex.Message);
